...
export const motoristaResolvers = {
  ...
  Mutation: {
    createMotorista: async (_: any, args: any) => {
      ...
      return await prisma.motorista.create({
        data: {
          nome,
          email,
          fotoMotorista, // Mapear para snake_case do banco
          cpf,
          cnh,
          vCnh: new Date(vCnh), // Mapear para snake_case
          tipoMotorista, // Mapear para snake_case
          operadoraId: parseInt(operadoraId), // Mapear para snake_case
        },
      });
    },
    updateMotorista: async (_: any, { id, data }: any) => {
    ...
    deleteMotorista: async (_: any, { id }: any) => {
